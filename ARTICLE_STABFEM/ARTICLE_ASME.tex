...
Gallaire et al. (FDR 2016). The derivation is based on a multiple scale decomposition based on the parameter $\epsilon = 1/Re - 1/Re_c$, and the slow time scale $\tau = \epsilon t$. The starting point is an expansion of the flow under the following form :

\begin{eqnarray}
{\bf u} &=& {\bf u}_{bc} + \epsilon \left[ A(\tau) \hat{\bf u}_c e^{i \omega_c t} + c.c. \right] \label{WNL1}\\
&+& \epsilon^2 \left[ {\bf u}_\epsilon + |A|^2  {\bf u}_{2,0} + \left(  A^2 {\bf u}_{2,2} e^{2 i \omega_c t} + c.c. \right) \right] + {\cal O}( \epsilon ^3)
\nonumber
\end{eqnarray}
where ${\bf u}_{bc}$ is the base flow at the threshold $Re_c$, $\hat{\bf u}$ the neutral eigenmode at $Re_c$, $A(\tau)$ a slowly varying amplitude.
...
form :
\be{WNL3}
\frac{\partial A }{\partial \tau} = \Lambda A - (\nu_0+\nu_2)  |A|^2 A
\ee
With 
\begin{eqnarray}
...
$|\tilde{A}| = \sqrt{\Lambda_r/(\nu{0,r}+\nu{2,r})}$ and eventually, reintroducing the scaling :
\be 
|A| =  \frac{\Lambda_r}{\nu{0,r}+\nu{2,r} } \sqrt{\frac{1}{Re_c}-\frac{1}{Re}}
\ee

The approach gives good prediction in the imediate vicinity of $Re_c$, but deviation appears very 
rapidly and disagreement is already large at $Re=48$ (although, as discussed by Galaire et al. 
and also observed by Tchoufag et al. (PRL), a convenient definition of the $\epsilon$ improves the results).
...
\end{center}
\caption{SELF CONSISTENT APPROACH FOR THE WAKE OF A CYLINDER, $Re = 100$. NOTE THAT THE AMPLITUDE IS NOTABLY LOWER THAN IN MANTIC-LUGO. TO BE EXPLAINED.}
\label{fig:SC100}
\end{figure}
