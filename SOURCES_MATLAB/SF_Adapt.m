...
uniqueFields = unique(char(cellfun(@(x)char(fieldnames(x)),{varargin{1:nfields}},'UniformOutput',false)),'rows');
for k=1:nfields
     for u=1:size(uniqueFields,1)
         fieldName = strtrim(uniqueFields(u,:));
         if ~isfield(varargin{k}, fieldName)
             varargin{k}.(fieldName) = [];
         end
     end
end
flowforadapt = [varargin{1:nfields}]; 
% END WORKAROUND 

designation = ['_adapt',num2str(flowforadapt(1).mesh.meshgeneration+1)];
...
if(i==1&&strcmp(varargout{i}.datatype,'BaseFlow')) %% if first field is a base flow we have to recompute it !
    mydisp(2,' SF_Adapt : recomputing base flow');
    baseflowNew = SF_BaseFlow(varargout{i}, 'type', 'POSTADAPT'); 
     if (baseflowNew.iter > 0)
     %  Newton successful : store base flow
     varargout{i} = baseflowNew; 
