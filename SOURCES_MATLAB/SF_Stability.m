...
if (isfield(baseflow, 'gamma')) gammaDefault = baseflow.gamma;
else gammaDefault = 0;
end;
addParameter(p, 'gamma', gammaDefault, @isnumeric);
if (isfield(baseflow, 'rhog')) rhogDefault = baseflow.gamma;
else rhogDefault = 0;
end;
addParameter(p, 'rhog', rhogDefault);
if (isfield(baseflow, 'nu')) nuDefault = baseflow.nu;
else nuDefault = 0;
end;
...
if (isempty(sigmaPrev)) sigmaPrev = p.Results.shift;
    sigmaPrevPrev = p.Results.shift;
end;
if (isempty(eigenvaluesPrev)) eigenvaluesPrev = [p.Results.nev:-1:1];
end;
...
if (strcmp(p.Results.PlotSpectrum, 'yes') == 1)
    ...
    h = plot(real(shift), imag(shift), 'o');
    hold on;
    for ind = 1:length(eigenvalues)
        h = plot(real(eigenvalues(ind)), imag(eigenvalues(ind)), '*');
        hold on;
