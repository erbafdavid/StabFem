...
if (~isempty(xydata))
    ...
    if strcmp(contourplt, 'off')
        ...
        if (strcmp(dashednegative, 'yes'))
            ...
            if (length(isopos) == 1) isopos = [isopos, isopos];
            end;
            isoneg = isolevels(isolevels < 0);
            if (length(isoneg) == 1) isoneg = [isoneg, isoneg];
            end;
            [~, hhc] = contour(X, Y, C, isoneg, '--', 'LineColor', [0, 0, 0]);
            hh = [hh; hhc];
            [~, hhc] = contour(X, Y, C, isopos, 'LineColor', [0, 0, 0]);
            hh = [hh; hhc];
        else
