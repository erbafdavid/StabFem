...
\includegraphics[width=.9 \linewidth]{Cylinder_Cx_baseflow.png}
...
Definition of lift and drag operators :



\be{drag}
F_x ({\bf u},p) = 
 \int_{\Gamma_w} \left[-p {\bf n} + \frac{2}{Re} {\bf D}({\bf u}) \cdot {\bf n} \right]   \cdot {\bf e_x} 
\ee


\be{lift}
F_y ({\bf u},p) = 
 \int_{\Gamma_w} \left[-p {\bf n} + \frac{2}{Re} {\bf D}({\bf u}) \cdot {\bf n} \right]   \cdot {\bf e_y} 
\ee
...
The resulting mesh is used for the rest of the computation presented in this paper (except for displaying the eigenmode in figure 4$a$). 
...
$\partial_t  {\bf u }  = NS(  {\bf u } )$, therefore dropping the systematic reference of the incompressibility constraint and associated pressure field.
...
We give here a brief account of the Weakly nonlinear model of Sipp \& Lebedev, also discussed by
Gallaire et al. (FDR 2016). The derivation is based on a multiple scale decomposition based on the parameter $\epsilon^2 = 1/Re - 1/Re_c$, and the slow time scale $\tau = \epsilon t$. The starting point is an expansion of the flow under the following form :
...
\nonumber
\end{eqnarray}
where ${\bf u}_{bc}$ is the base flow at the threshold $Re_c$, $\hat{\bf u}$ the neutral eigenmode at $Re_c$, $A_{wnl}(\tau)$ a slowly varying amplitude.
The order $\epsilon^2$ contains three terms respectively computed as the solutions of the following linear problems :
\begin{eqnarray}
NSL_{{\bf u}_bc} ({\bf u}_\epsilon) + 2 \nabla \cdot {\bf D}( \hat{\bf u}) &=&0,\\
NSL_{{\bf u}_bc} ({\bf u}_{2,0}) &=& {\cal C}(\hat{\bf u}, ,\overline{\hat{\bf u}}),\\
NSL_{{\bf u}_bc} ({\bf u}_{2,2}) - 2 i \omega_c {\bf u}_{2,2}  &=& \frac{1}{2} {\cal C}(\hat{\bf u},\hat{\bf u}).
 \end{eqnarray}

Finally compatibility conditions are imposed at order 3 leading to an amplitude equation with the following
form :
\be{WNL3}
...
As can be seen on figures 6 and 7,  the approach gives good prediction in the immediate vicinity of $Re_c$, but deviation appears very rapidly and disagreement is already large at $Re=48$ (although, as discussed by Gallaire et al. and also observed by Tchoufag et al. (PRL), a convenient definition of the $\epsilon$ improves the results).


\paragraph{Self-Consistent approach}

We now discuss the self-consistent approach as introduced by Mantic-Lugo et al.
...
Where ${\bf u }_m$ is a {\em mean flow} defined by phase-averaging over the cycle, $\tilde{{\bf u }}_1$ is a pseudo-eigenvector which is normalized by the condition  $||\tilde{{\bf u }}_1|| = 1/\sqrt{2}$, 
 $A$ is an amplitude parameter directly related to the energy of the oscillating flow, and $\lambda = \sigma_{SC} + i \omega_{SC}$ is a pseudo-eigenvalue which depends upon the parameter $A$. 

\be{HBEq0}
NS(  {\bf u }_m ) - A_{sc}^2 {\cal C}( \tilde{{\bf u }}, \overline{\tilde{\bf u }}) = 0,
\ee
\be{HBEq1}
(\sigma_{sc} + i \omega_{sc}) \tilde{{\bf u }} =  NSL_{{\bf u }_m}(  \tilde{{\bf u }} ),
\ee
\be{BBEqnorm}
|| \tilde{{\bf u }}|| = \frac{1}{\sqrt{2}}.
\ee
...
As discussed above, the Self-Consistent model was initially presented as the authors as a rational way to explain the dynamics in terms of a stability analysis of the {\em mean flow}. Furthermore, the resolution method presented by the authors, which involves a double iteration loop, is not easy to implement and not particularly efficient.
...
\paragraph{Implementation and results for cylinder}
...
Figure \ref{fig:listingNL}, which is again extracted from the script 
\verb|SCRIPT\_CYLINDER\_DEMO.m|, illustrate the sequence of commands to perform both the weakly nonlinear and the harmonic balance studies for the case of a cylinder.
...
On line 2 we then compute all the terms and coefficients of the wnl model. Then we construct  vectors to store the frequency and the amplitude of the limit cycle in the range $Re = [47.5-100]$. Note that for the computation at $Re=47.5$ we need a guess for the amplitude, which is deduced from the weakly nonlinear approach on line 9. In the sequel of the loop no guess value is prescribed, hence the solver uses continuation.
...
\end{center}
\caption{Harmonic-balance results : characteristics of the limit cycle.
NOTE THAT THE AMPLITUDE IS NOTABLY LOWER THAN IN MANTIC-LUGO. TO BE EXPLAINED.
}
\label{fig:HB}
\end{figure}
...
The whole results are presented in figures 8 (mean-flow properties) and 9 (oscillating mode properties).
...
\end{center}
\caption{SELF CONSISTENT APPROACH FOR THE WAKE OF A CYLINDER, $Re = 100$.}
\label{fig:SC100}
\end{figure}
...
The Reference mesh is adapted to the Structural sensitivity, for $Re= 60$, with interpolation error 0.01, mesh dimensions $[-40,120]\times[0,40]$, and no-stress boundary conditions at the outlet and lateral boundaries.


Figure \ref{fig:MeshConvergence} displays the results.
