...
%bf.xlim = [-1.5 4.5]; bf.ylim=[0,3];
figure();SF_Plot(bf,'ux','Contour','on','CLevels',[0 0],'xlim',[-1.5 4.5],'ylim',[0 3]);
%SF_Plot(bf,'ux');
...
em.xlim = [-2 8]; em.ylim=[0,5];
figure();SF_Plot(emA,'ux1Adj','xlim',[-2 8],'ylim',[0 5]);
...
figure();SF_Plot(emS,'sensitivity','xlim',[-2 4],'ylim',[0 3]);
%title('Structural sensitivity for Re=60')
box on; pos = get(gcf,'Position'); pos(4)=pos(3)*AspectRatio;set(gcf,'Position',pos); % resize aspect ratio
set(gca,'FontSize', 18);
saveas(gca,'FIGURES/Cylinder_SensitivityRe60',figureformat);
figure();SF_Plot(emE,'endogeneity.re','xlim',[-2 6],'ylim',[-3 3]);
hold on;SF_Plot(emE,'endogeneity.im','xlim',[-2 6],'ylim',[-3 3],'symmetry','XM');
%title('Structural sensitivity for Re=60')
box on; pos = get(gcf,'Position'); pos(4)=pos(3)*AspectRatio;set(gcf,'Position',pos); % resize aspect ratio
set(gca,'FontSize', 18);
saveas(gca,'FIGURES/Cylinder_SensitivityRe60',figureformat);
