...
for i = 1:length(meshdir)
    ...
    nv=headerline{1};nt=headerline{2};ns=headerline{3};    
    disp([name,blanks(25-length(name)), ' | ' date,blanks(20-length(date)) , ' | ',num2str(nv) ]);
end
disp(' ');disp('     (list of base flows associated to newly computed meshes ; couples of .txt/.ff2m files )');
disp(' ');
disp('Name                      | Date                 | Size (bytes) ');%| (Number of vertices) ');
meshdir = dir([ffdatadir,'MESHES/*.txt']);
meshdir=nestedSortStruct(meshdir,'datenum');
for i = 1:length(meshdir)
    name = meshdir(i).name;date = meshdir(i).date;    
...
disp('     (list of base flows compatible with current mesh ; couples of .txt/.ff2m files )');disp(' ');
disp('Name                      | Date                 | Size ');
