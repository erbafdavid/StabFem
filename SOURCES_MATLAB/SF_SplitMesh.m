function flowfield = SF_SplitMesh(varargin)
% 
...
% launch ff++ code        
        ...
        error = 'ERROR : FreeFem splitmesh aborted';
...
%  not .ff2m (which are not regenerated by split)
    
    % initialise structure and position input files
    flowfieldNew = flowfield;
    flowfieldNew.mesh =  ffmesh;
    flowfieldNew.filename=[ffdatadir 'MESHES/BaseFlow_split_Re' num2str(flowfield.Re) '.txt']
    
    % launch SF_Baseflow in 'NEW' mode
    flowfieldNew = SF_BaseFlow(flowfieldNew,'type','NEW');
    
     if(flowfieldNew.iter>0)
     %%% NB : WE HAVE TO FIND A BETTER ERROR MANAGEMENT SYSTEM !
		%  Newton successful : store base flow
		flowfield=flowfieldNew;
