...
switch (baseflow.mesh.problemtype)
    ...
    case ('AxiXRCOMPLEX') % Newton calculation for axisymmetric base flow WITH COMPLEX MAPPING
        ...
        if(length(p.Results.MappingParams)==6) % if no parameters are specified then the file must already exist
        fid = fopen('Param_Mapping.edp', 'w');
            fprintf(fid, '// Parameters for complex mapping (file generated by matlab driver)\n');
            fprintf(fid, ['real ParamMapLm = ', num2str(p.Results.MappingParams(1)), ' ;']);
            fprintf(fid, ['real ParamMapLA = ',  num2str(p.Results.MappingParams(2)), ' ;']);
            fprintf(fid, ['real ParamMapLC = ', num2str(p.Results.MappingParams(3)), ' ;']);
            fprintf(fid, ['real ParamMapGC = ',  num2str(p.Results.MappingParams(4)), ' ;']);
            fprintf(fid, ['real ParamMapyA = ', num2str(p.Results.MappingParams(5)), ' ;']);
            fprintf(fid, ['real ParamMapyB = ',  num2str(p.Results.MappingParams(6)), ' ;']);
        fclose(fid);
        end
