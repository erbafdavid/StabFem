...
parser.add_argument('@@levelsFilled', type=str,
                    help='The levels of the contour plot')

parser.add_argument('@@levelsFilled2', type=str,
                    help='The levels of the second contour plot')
...
parser.add_argument('@@BoxCM', type=str,
                    help='The option BoxCM to activate the rectangle')

parser.add_argument('@@BoxCMOrig', type=str,
                    help='The lower left coordinate of CM box')

parser.add_argument('@@BoxCMUpperCor', type=str,
                    help='The upper right coordinate of CM box')
...
try:
    ...
    options['BoxCM'] = False
    print("No option BoxCM selected")
    
try:
        inputvalue = args.BoxCMOrig
        inputlist = list(inputvalue.split(','))
        inputlist = [float(x) for x in inputlist]
        options['BoxCMOrig'] = np.array(inputlist)
except:
    print("No option BoxCMOrig selected")
    
try:
        inputvalue = args.BoxCMUpperCor
        inputlist = list(inputvalue.split(','))
        inputlist = [float(x) for x in inputlist]
        options['BoxCMUpperCor'] = np.array(inputlist)
except:
    print("No option BoxCMUpperCor selected")
    
try:
        inputvalue = args.levelsFilled
        inputlist = list(inputvalue.split(','))
        inputlist = [float(x) for x in inputlist]
        if (inputlist[0] != 0.0 or inputlist[1] != 0.0):
            options['LevelsFilled'] = np.array(inputlist)
        else:
            options['LevelsFilled'] = None
except:
    print("No option levelsFilled selected")

try:
    if(args.levelsFilled2 != "None"):
        inputvalue = args.levelsFilled2
        inputlist = list(inputvalue.split(','))
        inputlist = [float(x) for x in inputlist]
        if (inputlist[0] != 0.0 or inputlist[1] != 0.0):
            options['LevelsFilled2'] = np.array(inputlist)
        else:
            options['LevelsFilled2'] = None
except:
    print("No option levelsFilled2 selected")
